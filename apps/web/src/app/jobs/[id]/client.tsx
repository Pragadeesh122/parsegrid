...
export default function JobDetailClient({
  ...
  const approveMutation = useApproveModel(token ?? "");
  ...
  // Phase 7 idle states — SSE turns off and we stop polling.
  const isProcessing =
    !!job &&
  ...
  const handleApproveModel = async (editedModel: DatabaseModel) => {
    setIsSubmitting(true);
    setApproveError(null);
    try {
      await approveMutation.mutateAsync({jobId, model: editedModel});
      queryClient.invalidateQueries({queryKey: ["job", jobId]});
    } catch (e) {
      setApproveError(
  ...
  const handleRejectModel = async () => {
    ...
    setApproveError(null);
    try {
      await rejectMutation.mutateAsync(jobId);
    } catch (e) {
      setApproveError(
        e instanceof Error ? e.message : "Failed to reject model",
      );
    } finally {
      setIsSubmitting(false);
    }
  ...
  return (
    <AppShell>
      <div className='px-6 py-8 lg:px-10'>
        ...
        <div className='mt-8 justify-center max-w-11/12 items-center space-y-8'>
          {/* Progress */}
            ...
            <div className='rounded-2xl border border-zinc-800/60 bg-zinc-900/30 p-6'>
              ...
              <div className='mt-4 flex gap-3'>
                ...
                <button
                  ...
                  disabled={
                    !targetQuery.trim() || targetQueryMutation.isPending
                  }
          ...
          {/* Model Review (Phase 7) */}
          {(job.status === "MODEL_PROPOSED" ||
            job.status === "AWAITING_REVIEW") &&
            job.proposed_model && (
              <div className='rounded-2xl border border-zinc-800/60 bg-zinc-900/30 p-6'>
                <ModelReview
                  proposedModel={job.proposed_model}
                  onApprove={handleApproveModel}
                  onReject={handleRejectModel}
