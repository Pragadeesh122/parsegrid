...
export default function NewJobPage() {
  ...
  const handleSubmit = async () => {
    ...
    setIsUploading(true);
    try {
      // Phase 1: Direct upload through FastAPI
      // In production with Auth.js, the token would come from the session
      const formData = new FormData();
      formData.append("file", selectedFile);

      const uploadRes = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL || "http://localhost:8000"}/api/v1/upload/direct`,
        {
          method: "POST",
          body: formData,
