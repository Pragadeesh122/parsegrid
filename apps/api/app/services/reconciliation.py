...
Pipeline:
    ...
    2. entity_resolution           — LLM-assisted canonical-key assignment
                                     (fires only when PKs are non-unique)
    3. canonicalize_parents        — composite-natural-key dedupe w/ merge
    4. resolve_foreign_keys        — fill child FK columns from parent keys
...
def canonicalize_parents(
    ...
    for row in rows:
        ...
        if key in seen:
            ...
            if merged_fields:
                notes.append(
                    f"merged {table.table_name} duplicate for key {key}: "
                    f"filled {merged_fields}"
                )
            else:
                notes.append(
                    f"duplicate {table.table_name} row for key {key} had no new fields"
                )
            continue
...
def needs_resolution(rows: list[dict[str, Any]], pk_columns: list[str]) -> bool:
    """Cheap pre-check — returns True only when normalized PK tuples are non-unique.

    Exact duplicates after strip/lower are the signal that the same entity
    was extracted more than once (potentially with name-format variants across
    chunks).  When False, all PKs are already unique and no LLM call is needed.
    """
    keys = [
        tuple(row.get(col, "").strip().lower() for col in pk_columns)
        for row in rows
        if all(row.get(col) for col in pk_columns)
    ]
...
def entity_resolution(
    ...
    """Group duplicate rows by entity and return one LLM-merged row per entity.

    Handles both problems in a single call:
    - Same entity extracted with different PK string formats across chunks
      (e.g. "Last, First" vs "First Last") are grouped correctly.
    - Fields scattered across sparse and rich chunks are merged by the LLM,
      which picks the best non-null value per field rather than blindly
      filling nulls (avoiding accepting hallucinated values from other chunks).

    Uses gpt-4o-mini.  Fires only when ``needs_resolution()`` detects
    duplicate normalised PK tuples — no-ops for documents with clean unique
    PKs.

    On any LLM failure the original rows are returned unchanged so the
    pipeline degrades gracefully to field-by-field merge in
    ...
    for entity in entities:
        ...
        richest = max(
            source_rows,
            key=lambda r: sum(
                1 for k, v in r.items() if not k.startswith("__") and v is not None
            ),
        )
    ...
    # Pass through any rows the LLM didn't account for (safety net).
    for i, row in enumerate(rows):
    ...
    notes.append(
        f"entity_resolution({table.table_name}): {len(rows)} rows → "
        f"{len(merged_rows)} entities"
    )
    logger.info(
        f"entity_resolution({table.table_name}): {len(rows)} rows → "
        f"{len(merged_rows)} entities"
...
def reconcile_model(
    ...
    # 2. Entity resolution — assign __canonical_key to rows when the same
    #    entity appears with variant PK strings across extraction chunks.
    #    No-ops (skips LLM) when all PKs are already unique.
    ...
    # 3. Canonicalize each table on its primary key (merge on collision)
